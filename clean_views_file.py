#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Script para limpiar engine/views.py y agregar las funciones correctamente"""
...
# Buscar donde termina trading_loop_control_api (última función correcta)
# Buscar el patrón "status=500)" que cierra la función
import re

# Buscar el último "status=500)" antes de los comentarios corruptos
pattern = r'(.*?status=500\))\s*#.*?Funciones adicionales.*'
match = re.search(pattern, text, re.DOTALL)

if match:
    # Obtener todo hasta el último status=500) correcto
...
'''
    ...
    with open('engine/views.py', 'w', encoding='utf-8') as f:
        f.write(clean_text)
    print("✅ Archivo limpiado y funciones agregadas correctamente")
else:
    # Si no encuentra el patrón, buscar el último "status=500)" sin comentarios
    pattern2 = r'(.*status=500)\)'
    matches = list(re.finditer(pattern2, text))
    if matches:
        last_match = matches[-1]
        clean_text = text[:last_match.end()]
...
'''
        ...
        print("❌ No se pudo encontrar el final de trading_loop_control_api")
